...
u8 gt1x_config_factory_check[GTP_CONFIG_MAX_LENGTH] = { 0 };
#endif
...
#if GN_MTK_BSP_DEVICECHECK
#define TP_MODULE {"Amoled","ERROR","ERROR","ERROR","ERROR", "ERROR"}
#endif
...
//Gionee BSP1 yaoyc 20150420 add for cancel touch key when slide up begin
#if GN_TPD_CANCEL_TK_SLIP_UP
static u16 gt1x_touch_key_status[3] = {0,};   //record key status , bit 0 is key_array[0];..
static struct delayed_work key_work;
//static bool is_cancel_key = false;
extern struct tpd_device *tpd;
static u16 slide_y[5] = {0,};
static u16 cur_count = 0;
#define Y_BOTTOM  1230  // = max_y - 50
#define SLIDE_DELTA_Y 3  // be used to check slide distance
#define KEY_WORK_DELAY_TIME 22 // key up event delay time , 30 ms
#endif
//Gionee BSP1 yaoyc 20150420 add for cancel touch key when slide up end
...
//Gionee BSP1 yaoyc 20150420 add for cancel touch key when slide up begin
#if GN_TPD_CANCEL_TK_SLIP_UP
static bool is_swip_up_touch(void)
{
	int i;
	printk("%s,cur_count = %d\n",__func__,cur_count);
	if(cur_count == 0)
		return false;
	for(i = 0;i <cur_count;i++) {
		printk("%s,i=%d,y=%d\n",__func__,i,slide_y[i]);
	}
	//if(slide_y[0]>Y_BOTTOM&&(slide_y[0]-slide_y[cur_count])>SLIDE_DELTA_Y)
	//if(slide_y[0]>Y_BOTTOM&&(slide_y[0] > slide_y[cur_count]))
	if(slide_y[0]>Y_BOTTOM)
		return true;
	return false;
	
}
static void gt1x_check_key_work_func(struct work_struct *work)
{
	int i = 0;
	bool is_cancel_key = false;
	printk("%s,enter\n",__func__);
	is_cancel_key = is_swip_up_touch();
	if(is_cancel_key) {
		for(i=0;i<3;i++) {
			printk("%s, need cancel ; id=%d,status=%d\n",__func__,i,gt1x_touch_key_status[i]);
			if(gt1x_touch_key_status[i] == 1) {
				input_event(tpd->dev,EV_KEY,gt1x_touch_key_array[i],-1);
				gt1x_touch_key_status[i]=0;
			}
		}
	} else {
		for(i=0;i<3;i++) {
			printk("%s, id=%d,status=%d\n",__func__,i,gt1x_touch_key_status[i]);
			if(gt1x_touch_key_status[i]==1) {
				input_report_key(tpd->dev,gt1x_touch_key_array[i],0);
				gt1x_touch_key_status[i]=0;
			}
		}
	}
...
static int goodix_register_hardware_info(u8 sensor_id,  u8 **id_2_name)
{
	u8 fireware_id[2] = { 0 };
	u8 resolution[4] = { 0 };
	...
	ret = gt1x_i2c_read(GTP_REG_FIREWARE_ID, fireware_id, 2);
	if (ret < 0) {
		return ret;
	}

	ret = gt1x_i2c_read(GTP_REG_RESOLUTION_START_ADDR, resolution, 4);
	if (ret < 0) {
		return ret;
	}
	
	resolution_x = resolution[1] *256 + resolution[0] ;
	resolution_y = resolution[3] *256 + resolution[2] ;
	
	gn_tp_hw_info.gn_dev_type = GN_DEVICE_TYPE_TP;

	snprintf(gn_tp_hw_info.name, GN_DEVICE_NAME_LEN, "%s[%d]-%x-[%d, %d]\n",  
		id_2_name[sensor_id],sensor_id, fireware_id[0],resolution_x, resolution_y);
...
s32 gt1x_init_panel(void)
{
  ...
  #if GN_MTK_BSP_DEVICECHECK
	u8 *tp_module[6] = TP_MODULE;
  #endif
   
  #if GTP_FACTORY_CHECK
	u8 cfg_len_factory_check = 0;
  #endif
  ...
  #if GTP_FACTORY_CHECK
	const u8 cfg_grp0_factory_check[] = GTP_CFG_GROUP0_FACTORY_CHECK;
	const u8 cfg_grp1_factory_check[] = GTP_CFG_GROUP1_FACTORY_CHECK;
	const u8 *cfgs_factory_check[] = {
		cfg_grp0_factory_check, cfg_grp1_factory_check
	};
	u8 cfg_lens_factory_check[] = {
		CFG_GROUP_LEN(cfg_grp0_factory_check),
		CFG_GROUP_LEN(cfg_grp1_factory_check)
	};

	printk("Config Groups factory_check Length: %d, %d", cfg_grp0_factory_check[0], cfg_grp0_factory_check[1]);
  ...
  #if GTP_FACTORY_CHECK
	cfg_len_factory_check = cfg_lens_factory_check[sensor_id];

	printk("CTP_CONFIG_GROUP%d used, gt1x_config length: %d", sensor_id, cfg_len_factory_check);

	if (cfg_len_factory_check < GTP_CONFIG_MIN_LENGTH || cfg_len_factory_check > GTP_CONFIG_MAX_LENGTH) {
	...
	memset(gt1x_config_factory_check, 0, sizeof(gt1x_config_factory_check));
	memcpy(gt1x_config_factory_check, cfgs_factory_check[sensor_id], cfg_len_factory_check);

	/* clear the flag, avoid failure when send the_config of driver. */
	gt1x_config_factory_check[0] &= 0x7F;
	...
	#if GN_MTK_BSP_DEVICECHECK
	ret = goodix_register_hardware_info(sensor_id, tp_module);
	if (ret < 0) {
		return -1;
	}
    #endif
...
static s32 gt1x_wakeup_sleep(void)
{
	...
	while (retry++ < 2) {
...
#endif
		{
			...
			// Gionee liudj add for CR01544754
			gesture_doze_status = DOZE_DISABLED;
...
void gt1x_power_reset(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if(gt1x_smart_cover_update_state()) {
			msleep(500);
			continue;
		}
		break;
...
s32 gt1x_request_event_handler(void)
{
	...
	case GTP_RQST_CONFIG:
		...
		ret = gt1x_smart_cover_update_state();
		if (ret) {
...
#if GN_TPD_CANCEL_TK_SLIP_UP		
		if(key_value > 0) {
			input_report_key(dev,gt1x_touch_key_array[key_value/2],1);
			gt1x_touch_key_status[key_value/2] = 1;
		} else {
			queue_delayed_work(gt1x_workqueue,&key_work,msecs_to_jiffies(KEY_WORK_DELAY_TIME));
		}
#else
...
static struct kobj_attribute ps_attrs[] = {
	__ATTR(enable,0664, gt1x_ps_enable_show, gt1x_ps_enable_store),
	__ATTR(state, 0664, gt1x_ps_state_show, gt1x_ps_state_store)
};
...
ssize_t smart_cover_show(struct device *kobj, struct device_attribute *attr, char *buf)
{
    struct smart_cover_device *dev = gt1x_sc_dev;
...
ssize_t smart_cover_store(struct device *kobj, struct device_attribute *attr, const char *buf, size_t count)
{
    struct smart_cover_device *dev = gt1x_sc_dev;
    int s = (buf[0] - '0');
...
static struct kobj_attribute sc_attr = 
    __ATTR(state, 0664, smart_cover_show, smart_cover_store);
#endif
...
{
	u8 end_cmd = 0;//For gesture check sum error.
	s32 ret = -1;
...
int gt1x_resume(void)
{
    ...
    if (!gesture_enabled && !wake_switch && !gt1x_halt)
        return 0;
#else
    if (!gt1x_halt)
        return 0;
...
//Gionee BSP1 yaoyc 20150417 add for check key status and cancel touch key begin
#if GN_TPD_CANCEL_TK_SLIP_UP
	INIT_DELAYED_WORK(&key_work,gt1x_check_key_work_func);
#endif
//Gionee BSP1 yaoyc 20150417 add for check key status and cancel touch key end
