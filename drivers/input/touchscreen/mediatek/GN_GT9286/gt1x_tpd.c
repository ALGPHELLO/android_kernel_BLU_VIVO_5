...
int gt1x_power_switch(s32 state)
{
	...
	case SWITCH_ON:
		...
		//msleep(100);
                GTP_GPIO_EN_OUTPUT(GTP_EN_PORT, 1);
		power_state = 1;
	...
	case SWITCH_OFF:
		GTP_DEBUG("Power switch off!");
		GTP_GPIO_EN_OUTPUT(GTP_EN_PORT, 0);
		power_state = 0;
...
{
	int ret = 0;
	char temp_data[SENSOR_A_PORT_SIZE*2];
	u16 *p = &node_value[0];
	...
	ret = gt1x_i2c_read(0xb53C, temp_data, rawdata_size);
	if(ret < 0) {
...
static DEVICE_ATTR(factory_check, 0664, show_factory_check, NULL);
static DEVICE_ATTR(factory_check_value,0664, show_factory_check_value, NULL);
#endif
...
#if GTP_SMART_COVER
extern  ssize_t smart_cover_show(struct device *kobj, struct device_attribute *attr, char *buf);
extern  ssize_t smart_cover_store(struct device *kobj, struct device_attribute *attr, const char *buf, size_t count);
...
//GIonee: zhouzq 2015-12-30 add the debug log
            GTP_DEBUG("gesture handler end");
			continue;
		}
#endif
		if (gt1x_halt) {
