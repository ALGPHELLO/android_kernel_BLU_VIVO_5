...
s32 gesture_event_handler(struct input_dev * dev)
{
    ...
    if ((wake_switch != 0) && (doze_buf[0] == 0xCC)) {
			doze_buf[0] = 0;
			gt1x_i2c_write(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);

			GTP_INFO("Gesture double wake \n");
			
			input_report_key(dev, KEY_DOUBLE_WAKE, 1);
			input_sync(dev);
			input_report_key(dev, KEY_DOUBLE_WAKE, 0);
			input_sync(dev);
			goto clear_reg;
    }
    #endif
   ...
   if(0 == gesture_enabled)
    {
       GTP_DEBUG("gesture_enabled is disabled");
       goto clear_reg;
    }
