...
//Gionee map 20160726 modify 	for CR01726691 begin
#ifdef ORIGINAL_VERSION
		.framelength = 2000,			//record different mode's framelength
#else		
		.framelength = 3037,//2000,			//record different mode's framelength
#endif
//Gionee map 20160726 modify	for CR01726691 end
...
.framelength = 3037, //2025,
	...
	//pangfei modify for CR01662226 start
	#if 0
	.normal_video = {
		.pclk = 179200000,
		.linelength = 2950,
...
//Gionee map 20160726 modify	for CR01726691 begin
	...
	#endif
	//pangfei modify for CR01662226 end	
	.hs_video = {
		.pclk = 179200000,
		.linelength = 2950,
		.framelength = 506,
...
//Gionee BSP1 yanggy 20140930 add for Cam_Performance @CR01392111 begin
#define S5K5E2YA_Use_OTP
#ifdef S5K5E2YA_Use_OTP
extern int ofilm_get_otp_module_id(int *id);
extern bool ofilm_otp_lenc_update(void);
extern bool S5K5E2YA_OFILM_MIPI_update_wb_register_from_otp(void);
extern int S5K5E2YA_OFILM_MIPI_read_otp_wb(struct S5K5E2YA_OFILM_MIPI_otp_struct *otp);
extern void S5K5E2YA_OFILM_MIPI_write_otp_wb(struct S5K5E2YA_OFILM_MIPI_otp_struct *otp);
extern void S5K5E2YA_OFILM_MIPI_algorithm_otp_wb1(struct S5K5E2YA_OFILM_MIPI_otp_struct *otp);

//static struct S5K5E2YA_OFILM_MIPI_otp_struct S5K5E2YA__Current_OTP;
static bool S5K5E2YA_OFILM_MIPI_OTP_Read_Flag = FALSE;
#endif
//Gionee BSP1 yanggy 20140930 add for Cam_Performance @CR01392111 end
...
{{ 2576, 1936,	  8,	8, 2560, 1920, 1280,  960, 0000, 0000, 1280,  960,	  0,	0, 1280,  960}, // Preview 
 ...
 #else
 { 2576, 1936,	  8,	8, 2560, 1920, 1280,  960, 0000, 0000, 1280,  960,	  0,	0, 1280,  960},//normal video
#endif
//pangfei modify for CR01662226 end
...
//Gionee <liu_hao> <2015-07-20> modify for camera mirror&flip begin
#ifdef ORIGINAL_VERSION 
#else
        write_cmos_sensor(0x0101,0x03);      
#endif
//Gionee <liu_hao> <2015-07-20> modify for camera mirror&flip end
	...
	//add by alex,20140718
	
	write_cmos_sensor(0x0B00,0x01);       //  GAS bypass ,check 
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				printk("s5k5e2ya_camera_sensor i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
//				return ERROR_NONE;
                goto S5K5E2YA_READ_SENSOR_ID_END;
			}	
			//LOG_INF("Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			printk("s5k5e2ya_camera_sensor Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
	}
S5K5E2YA_READ_SENSOR_ID_END:
...
#ifdef S5K5E2YA_Use_OTP
    sensor_init();
    ofilm_get_otp_module_id(&module_id);
	//LOG_INF("ofilm_get_otp_module_id ID = 0x%02x.\n",module_id);
#if 0
    if(S5K5E2YA_OFILM_MIPI_read_otp_wb(&S5K5E2YA__Current_OTP) != 0)
    {
        LOG_INF("read otp data err !!!\n");
        S5K5E2YA_OFILM_MIPI_OTP_Read_Flag = FALSE;
    }
    else
    {
        LOG_INF("read otp data success !!!\n");
        S5K5E2YA_OFILM_MIPI_OTP_Read_Flag = TRUE;
    }
#else
...
//#ifdef GN_MTK_BSP
#if 1
    //LOG_INF("R_to_G : 0x%x, B_to_G: 0x%x, G_to_G: 0x%x \n",S5K5E2YA__Current_OTP.R_to_G,S5K5E2YA__Current_OTP.B_to_G,S5K5E2YA__Current_OTP.G_to_G);
    if(S5K5E2YA_OFILM_MIPI_OTP_Read_Flag == TRUE)
    {
        ofilm_get_otp_module_id(&moudile_id);
        ofilm_otp_lenc_update();
		S5K5E2YA_OFILM_MIPI_update_wb_register_from_otp();
        //S5K5E2YA_OFILM_MIPI_algorithm_otp_wb1(&S5K5E2YA__Current_OTP);
        //S5K5E2YA_OFILM_MIPI_write_otp_wb(&S5K5E2YA__Current_OTP);
    }
    else
    {
        ofilm_otp_lenc_update();
